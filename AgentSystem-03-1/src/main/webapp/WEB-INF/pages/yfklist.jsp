...
<div class="searchuserdiv ope">
	<ul>
		...
		<option <c:if test="${accountDetail.detailType == 9996 }">
			...
			<c:forEach items="${accountConfigList }">
				<option
				<c:if test="${accountDetail.detailType == id }">
					selected = "selected"
				</c:if>
				 value="${id }">${configTypeName }</option>
			</c:forEach>
		</select>
		操作时间：
		<input class="Wdate" size="15" name="accountDetail.startTime" id="starttime" value="${accountDetail.startTime }" readonly="readonly"  type="text" onClick="WdatePicker()" />
		至

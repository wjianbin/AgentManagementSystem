$().ready(function(){
	$(".viewCustom").click(function(){
		var c = $(this);
		var c_id = c.attr('id');
		window.location.href="/viewcustom.action?id="+c_id;
	});
	...
	$(".modifyCustomStatus").click(function(){
		...
		if(c_status == '0'){
			if(confirm("确定要停用【"+c_customname+"】这个客户吗？")){
				$.post("/modifycustomstatus.action",{'id':c_id,'customStatus':c_status},function(result){
					if("success" == result){
						alert("停用成功！");
	...
	$(".modifyCustom").click(function(){
		...
		window.location.href="/modifycustom.action?id="+c_id+"&cname="+cname;
	});

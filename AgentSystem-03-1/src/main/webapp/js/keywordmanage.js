...
//		alert("id====="+kid);
		if(confirm("为了数据安全，需要您两次确认才能删除，您确定要删除关键词【"+keyword+"】吗（一次）？")){
			if(confirm("为了数据安全，需要您两次确认才能删除，您确定要删除关键词【"+keyword+"】吗（两次）？")){
				$.post('/deletekeyword.action',{'id':kid},function(result){				
					if("success" == result){
	...
	$(".xufei").click(function(){
		...
		var kid = obj.attr("kid");
		var keyword = obj.attr("keyword");
		ymPrompt.win({message:'/xufei.action?id='+kid,width:600,height:400,title:'当前为【'+keyword+'】进行续费操作',handler:callBack,iframe:true})
	});
});

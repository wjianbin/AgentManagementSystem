	...
	$(".checkselect").change(function(){
		...
		if(v == 1){
			if(confirm("您确定要将["+obj.attr("keyword")+"]的状态修改为审核中吗？")){
				$.post('/updatekeyword',{'id':kid,'checkStatus':1},function(result){
					if("success" == result){
			...
			if(confirm("您确定要将["+obj.attr("keyword")+"]的状态修改为不通过吗？")){
				$.post('/updatekeyword',{'id':kid,'checkStatus':3},function(result){
					if("success" == result){
			...
			if(confirm("您确定要修改["+obj.attr("keyword")+"]的使用状态为不使用吗？")){
				$.post('/updatekeyword',{'id':kid,'isUse':0},function(result){
					if("success" == result){
			...
			if(confirm("您确定要修改["+obj.attr("keyword")+"]的使用状态为使用吗？")){
				$.post('/updatekeyword',{'id':kid,'isUse':1},function(result){
					if("success" == result){

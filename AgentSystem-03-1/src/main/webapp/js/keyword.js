	...
	$("#keyword").blur(function(){
		...
		if(keyword.val() == '' || keyword.val().length <= 0){
			...
			$.post('/valikey.action',{'keywords':keyword.val()},function(result){
				if('success' == result){				
		...
		if(userid <= 0){
			...
			$.post('/valikey.action',{'keywords':$("#keyword").val()},function(result){
				if('success' == result){
					...
					$.post('/submitkeyword.action',{p:serviceType+"-"+years,'keywords':$("#keyword").val(),'customId':userid,'customName':userName},function(result){
						if("exception" == result){
